...
var generateDocumentParentDifferentAddress = miss.through(function (chunck, encoding, callback) {
  ...
  var options = {
    templateData: item.distribution.parentsDifferentAddressNotification.data,
    templateFilepath: item.distribution.parentsDifferentAddressNotification.template,
  ...
  createPdfFromTemplate(options, function (error, data) {
    if (error) {
      ...
      item.distribution.parentsDifferentAddressNotification.document = documentPath
    }
