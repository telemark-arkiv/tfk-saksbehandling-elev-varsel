...
var setupArchive = miss.through(function (chunck, encoding, callback) {
  ...
  // Varsel - orden - Berit Oline Skau - 1TIB - Porsgrunn videregående skole - 2016 - 1. termin

  archive._id = item._id
  archive.title = item.documentType + ' - ' + item.documentCategory + ' - ' + item.studentName + ' - ' + item.studentMainGroupName + ' - ' + item.schoolName + ' - ' + now.getFullYear() + ' - ' + item.period
