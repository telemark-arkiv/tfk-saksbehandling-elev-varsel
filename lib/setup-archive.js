...
var setupArchive = miss.through(function (chunck, encoding, callback) {
  ...
  archive.title = capitalize(item.documentType) + ' - ' + item.documentCategory + ' - ' + item.studentName + ' - ' + item.studentMainGroupName + ' - ' + item.schoolName + ' - ' + now.getFullYear() + ' - ' + item.period.replace(/\s+/g, '')
  ...
  item.distribution.restrictedAddressArchive = JSON.parse(JSON.stringify(archive))

  item.distribution.noGuardianFoundArchive = JSON.parse(JSON.stringify(archive))
